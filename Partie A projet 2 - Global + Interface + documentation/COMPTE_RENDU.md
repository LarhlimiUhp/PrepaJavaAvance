...
## 13. Relations entre Entités (One-to-Many)
Pour illustrer la gestion des relations complexes ("Association"), nous avons ajouté une entité `Catalogue`.

### A. Modèle Catalogue (`Catalogue.java`)
Placée dans `com.examen.stock.model`, elle contient une collection de produits.
...
### B. Mapping XML (`Catalogue.hbm.xml`)
Il définit la relation One-to-Many avec cascade.
...
### C. Relation Bidirectionnelle
Le modèle `Produit` a également été mis à jour pour inclure un champ `catalogue` (Many-to-One).

- **`Produit.java`** : Ajout de `private Catalogue catalogue;` et ses getters/setters.
- **`Produit.hbm.xml`** : Ajout de `<many-to-one name="catalogue" column="catalogue_id" class="Catalogue" />`.

Cette configuration permet de naviguer du catalogue vers ses produits, et d'un produit vers son catalogue parent.
