...
---
## 7. Étape 6 : Interface Graphique (JavaFX)

### Objectif
Fournir une interface visuelle conviviale pour gérer les produits et les catalogues. Cette étape utilise le modèle **MVC** (Modèle-Vue-Contrôleur) avec des fichiers **FXML** pour le design.

### Concepts Clés
- **FXML** : Fichier XML décrivant l'interface graphique (séparation design/code).
- **Controller** : Classe Java gérant la logique des événements (clic bouton, remplissage tableau).
- **Application** : Point d'entrée JavaFX (`Application.launch`) qui charge la scène.
- **ObservableList** : Liste spéciale qui met à jour la vue automatiquement quand les données changent.

### Code Source complet : Étape 6

#### 1. Vue Principale (`MainView.fxml`)
Utilise un `TabPane` pour séparer proprement la gestion des produits et celle des catalogues.

```xml
<BorderPane fx:controller="com.examen.stock.view.MainController">
    <center>
        <TabPane>
            <Tab text="Produits"><fx:include source="ProduitView.fxml"/></Tab>
            <Tab text="Catalogues"><fx:include source="CatalogueView.fxml"/></Tab>
        </TabPane>
    </center>
</BorderPane>
```

#### 2. Contrôleur de Produits (`ProduitController.java`)
Gère l'affichage dans une `TableView` et l'ajout de nouveaux produits via le `ProduitService`.
...
public class ProduitController {
    ...
    public void handleAjouterProduit() {
        ...
        service.enregistrerProduit(new Produit(nom, prix));
        handleRefresh();
    }
}
```

### Questions types : Étape 6
1. **Q : Quel est l'avantage d'utiliser FXML par rapport au code Java pur ?**
   - **R :** Cela permet de séparer proprement le design (XML) de la logique (Java), facilitant la maintenance et la collaboration.
2. **Q : À quoi sert l'annotation `@FXML` ?**
   - **R :** Elle permet au `FXMLLoader` d'injecter automatiquement les composants définis dans le XML (id) dans les champs correspondants de la classe contrôleur.
3. **Q : Pourquoi utiliser une `ObservableList` plutôt qu'une `ArrayList` dans les tables ?**
   - **R :** L'`ObservableList` notifie automatiquement la `TableView` de tout changement (ajout/suppression), ce qui évite de rafraîchir manuellement toute la vue.
